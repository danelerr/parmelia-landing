...
title: Payments API
description: Create stablecoin charges and receive signed onchain payment confirmations with the GatoPago API.
---
# GatoPago Payments API

Create a **Payment Intent**, open a **checkout link or QR**, and receive a signed
...
- The public Alpha uses `sk_test_…` keys and settles with test funds on Arbitrum
  Sepolia. Use test mode to build and to drive the
  [sandbox](#test-mode--sandbox).
- `sk_live_…` is reserved for a future mainnet environment and is not enabled
  in the public Alpha.
...
- Test mode settles on Arbitrum Sepolia. Mainnet mode is not enabled in the
  public Alpha.
...
| Chain (mainnet) | Not enabled in the public Alpha; Arbitrum One (`42161`) is the planned core environment. |
