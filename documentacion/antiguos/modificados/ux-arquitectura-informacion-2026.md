# GatoPago — arquitectura de información UX 2026

> **Estado actualizado el 23 de agosto de 2026:** se conserva como fuente de las reglas de arquitectura de información R-1 a R-8. Las referencias visuales de julio —paleta celeste, gradientes y clases antiguas— quedan reemplazadas por el rebranding maestro. Ante conflicto, prevalecen `../nuevos/gatopago-rebranding-maestro-2026.md` y la implementación verificada de la app.
>
> Versión original 1.0 — julio 2026. Documento de decisión, como CROSSCHAIN_DESIGN.md y
...
1. **Sistema de diseño real** en `index.css`: tokens de color, tipografía Recursive,
   bordes rectos, sombras pixel y semántica de botones (`.btn-primary` acción,
   `.btn-ghost` secundaria, `.btn-text` terciaria y `.btn-money` para el CTA
   financiero). `.btn-gradient` permanece únicamente como alias de compatibilidad;
   ya no representa un gradiente. El comportamiento de foco y movimiento se rige
   por la implementación actual y el rebranding maestro.
...
**P-6. Earn estaba fuera del sistema de botones en la captura de julio.** Usaba
clases custom en vez de `.btn` y era la única página así. Además su confirmación
era full-screen mientras Pay y Crosschain usaban
...
| R-3 | Cada pantalla tiene un solo CTA financiero principal, con `.btn-money` o el equivalente vigente de GatoPago UI. |
...
confirmación uniforme (R-4), estados pendiente/fallo honestos, borde Cat Fire
